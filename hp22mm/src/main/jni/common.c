...
{
    MAX_ASSERT(STATE_COUNT == sizeof(ph_state_string)/sizeof(char*));
...
{
    MAX_ASSERT(ERROR_COUNT == sizeof(ph_error_string)/sizeof(char*));
...
int pd_check_ph(char *function, PDResult_t result, int PenIdx)
{
    ...
    else
    {
        ...
        printf("ERROR: %s() failed: %s  PH state: %s  PH error: %d %s\n", function,
...
int DoPairing(int SupplyIdx, int PenIdx)
{
    ...
    {
            printf("ERROR: Cannot terminate pairing sessions\n");
            return -1;
    }
    ...
    for (step=1; step<=3; step++)
    {
        ...
        if(pd_r != PD_OK)
        {
            printf("ERROR: pd_pairing() step %d failed: %s\n", step, pd_get_error_description(pd_r));
            return -1;
        }
        if (status != KEY_NEGOTIATION_SESSION_OK && status != KEY_NEGOTIATION_SESSION_COMPLETE)
        {
            printf("ERROR: pd_pairing() step %d failed, status %d\n", step, status);
            return -1;
        }
    }
...
int DoOverrides(int SupplyIdx, int PenIdx)
{
    ...
    for (slot=0; slot<=1; slot++)
    {
        ...
        if (status != 0)
        {
            printf("ERROR: pd_set_secure_overrides() slot %d failed, status %d\n", slot, status);
            return -1;
        }
    }
    printf("Overrides COMPLETE\n");
    return 0;
}
...
int GetAndProcessInkUse(int PenIdx, int SupplyIdx)
{
    ...
    for (slot=0; slot<=1; slot++)
    {
        ...
        if (status != 0)
        {
            printf("ERROR: pd_ink_use() slot %d failed, status %d\n", slot, status);
            return -1;
        }
        ...
        if (status != 0)
        {
            printf("ERROR: ids_ink_use() from slot %d failed, status %d\n", slot, status);
            return -1;
        }
        ...
        if (status != 0)
        {
            printf("ERROR: pd_supply_status() slot %d failed, status %d\n", slot, status);
            return -1;
        }
    }
    return 0;
