// Modified to implement only crc-ccitt
...
#define _INITIAL_CRC    0xffff
#define _FINAL_XOR      0xffff
...
uint16_t lib_crc_ccitt_calc(const uint8_t* start, const uint8_t* end)
{
    ...
    crc = lib_crc_ccitt_init();

    for (p = start; p <= end; p++)
    {
        crc = lib_crc_ccitt_update(crc, *p);
    }
    return(lib_crc_ccitt_final(crc));
}
