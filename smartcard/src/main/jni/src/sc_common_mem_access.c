...
HP_SMART_CARD_result_t readTag0FamilyID(HP_SMART_CARD_device_id_t cardId, uint8_t *family_id) {
    return readByte(cardId, HP_SMART_CARD_FAMILY_ID, family_id);
// H.M.Wang 2021-3-31 修改为调用函数时根据设备号决定采用哪个函数，而不使用这个中间方法
...
//    }
//    return result;
// End of H.M.Wang 2021-3-31 修改为调用函数时根据设备号决定采用哪个函数，而不使用这个中间方法
}
